...
import "../../../../styles/teachers/phongdaotao/xemchitietdanhsachsinhvien.css";

export default function PhongDaoTaoXemChiTietSinhVien() {
  ...
  const [phongdaotaoSelectedValues, setPhongdaotaoSelectedValues] = useState<Record<string, string[]>>({});
  ...
  useEffect(() => {
    ...
    // Load Phòng đào tạo scores if available
    const savedPhongDaoTao = localStorage.getItem(`phongdaotaoScores_${classId}`);
    if (savedPhongDaoTao) {
      const parsed = JSON.parse(savedPhongDaoTao);
      setPhongdaotaoSelectedValues(parsed[studentId]?.phongdaotaoSelectedValues || {});
    }
  }, [studentId, classId]);
  ...
  // Tính điểm Phòng đào tạo
  const calcSectionTotalPhongDaoTao = (section: string) => {
    const sectionItems = diemData.filter(
      (item) =>
        item.muc === section ||
        item.mucCha === section ||
    ...
    const total = sectionItems.reduce((sum, item) => {
      let score = 0;
      if (item.loai === "counter") {
        const rawVal = phongdaotaoSelectedValues[item.muc]?.[0];
        const count = rawVal ? parseInt(rawVal) : 0;
        const diemMoiLan = parseInt(item.diem ?? "0");
        score = count * diemMoiLan;
      } else {
        const selected = phongdaotaoSelectedValues[item.mucCha || item.muc] || [];
        if (selected.includes(item.muc)) {
          score = parseInt(item.diem ?? "0");
        }
      }
  ...
  const calcAllTotalPhongDaoTao = () => {
    return bigSections.reduce((sum, section) => sum + calcSectionTotalPhongDaoTao(section), 0);
  };
      ...
      <table className="xemchitiet_students-table">
        ...
        <tbody>
            ...
            return (
              <React.Fragment key={section}>
                    ...
                    <tr key={item.muc}>
                      ...
                      <td style={{ fontWeight: "bold" }}>
                        {item.loai === "counter"
                          ...
                          : phongdaotaoSelectedValues[item.mucCha || item.muc]?.includes(item.muc)
                          ? item.diem
                          : ""}
                      </td>
                    </tr>
                ...
                <tr className="section-total">
                  ...
                  <td style={{ fontWeight: "bold" }}>{calcSectionTotalPhongDaoTao(section)}</td>
                </tr>
              </React.Fragment>
            );
          })}
          ...
          <tr className="all-total">
            ...
            <td style={{ fontWeight: "bold" }}>{calcAllTotalPhongDaoTao()}</td>
          </tr>
          {/* Xếp loại */}
          <tr className="rank-row">
            ...
            <td style={{ fontWeight: "bold" }}>{getRank(calcAllTotalPhongDaoTao())}</td>
          </tr>
        </tbody>
      </table>
